...
export function LanguagePicker({ slug, locale, onLocaleChange }: Props) {
  ...
  function selectLanguage(code: Locale, e: React.MouseEvent) {
  ...
  return (
    <details
      ref={detailsRef}
      className="relative z-[100] mx-auto mt-2 mb-1 w-full max-w-xs group"
    >
      ...
      <div
        className="absolute left-1/2 top-[calc(100%+6px)] z-[200] w-[min(100vw-2rem,20rem)] -translate-x-1/2 rounded-2xl border border-stone-200 bg-white p-2 shadow-2xl"
            ...
            return (
              <li key={lang.code}>
                <a
                  ...
                  onClick={(e) => selectLanguage(lang.code, e)}
                  className={`flex min-h-[48px] items-center gap-2 rounded-xl px-2.5 py-2 text-sm font-medium touch-manipulation no-underline ${
                    active
